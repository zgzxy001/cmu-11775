...
parser.add_argument("surf_feature_dir") # sound_out
...
def get_data(trainval_path, feature_dir):
    ...
    with open(trainval_path, 'r') as trainval_f:
        ...
        for line in tqdm(train_lines):
            ...
            file_path = Path(feature_dir+name+'.npy')
            bof_file_path = Path(bof_feature_dir + name + '.csv')
            if file_path.is_file() and bof_file_path.is_file():
                feature = np.load(feature_dir + name + '.npy')
                bof_feature = np.genfromtxt(bof_feature_dir + name + '.csv', delimiter=";", dtype="float")
                # print('feature = ', feature.shape)

                # feature.resize((54, 256))
                feature = np.mean(feature, axis=0)
                feature = np.append(feature, bof_feature)
        ...
        for line in tqdm(val_lines):
            ...
            if file_path.is_file() and bof_file_path.is_file():
                ...
                # feature.resize((54, 256))
                feature = np.mean(feature, axis=0)
                bof_feature = np.genfromtxt(bof_feature_dir + name + '.csv',
...
bof_feature_dir = args.surf_feature_dir
...
model.fit(train_dataset, validation_data=test_dataset, epochs=10, callbacks=[ckpt_callback,lr_scheduler])
...
print(matrix)
print(np.sum(predictions==val_label)/np.shape(predictions)[0])
